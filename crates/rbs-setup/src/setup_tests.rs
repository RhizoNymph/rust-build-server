...
#[test]
fn role_parses_new_names_and_legacy_aliases() {
    assert_eq!("client".parse::<Role>(), Ok(Role::Client));
    assert_eq!("server".parse::<Role>(), Ok(Role::Server));
    // the topology names replaced hardware names; the old spellings keep working
    assert_eq!("laptop".parse::<Role>(), Ok(Role::Client));
    assert_eq!("node0".parse::<Role>(), Ok(Role::Server));
    assert!(
        "worker"
            .parse::<Role>()
...
#[test]
fn generated_config_header_names_the_role() {
    assert!(generate_config(Role::Client).starts_with("# Generated by `rbs setup --role client`"));
    assert!(generate_config(Role::Server).starts_with("# Generated by `rbs setup --role server`"));
}

#[test]
fn generated_client_config_round_trips_with_client_sizing() {
    let cfg = parsed(&generate_config(Role::Client));
...
#[test]
fn generated_server_config_disables_remote_and_uses_big_sizing() {
    let cfg = parsed(&generate_config(Role::Server));
    assert_eq!(cfg.server.reserve_cores, 4);
...
fn setup_writes_config_when_absent() {
    ...
    let r = runner();
    setup_with(&r, &th.paths, &opts(Role::Client, &th)).expect("setup");
    let text = read(&th.paths.rbs_config());
    assert_eq!(text, generate_config(Role::Client));
}
...
fn existing_config_is_kept_without_force() {
    ...
    let r = runner();
    setup_with(&r, &th.paths, &opts(Role::Client, &th)).expect("setup continues");
...
fn force_overwrites_existing_config() {
    ...
    std::fs::write(&p, "[remote]\nhost = \"other\"\n").expect("write");
    let r = runner();
    let mut o = opts(Role::Server, &th);
    o.force = true;
    setup_with(&r, &th.paths, &o).expect("setup");
    assert_eq!(read(&p), generate_config(Role::Server));
}
...
fn kache_stats_without_remote_is_an_error() {
    ...
    let r = FakeRunner::new()
        ...
        .ok("kache", "")
        .ok("systemctl", "");
    let err = setup_with(&r, &th.paths, &opts(Role::Client, &th)).expect_err("must fail");
...
#[test]
fn server_installs_store_gc_timer_units() {
    ...
    th.write_minio_env("AK", "SK");
    let r = runner();
    setup_with(&r, &th.paths, &opts(Role::Server, &th)).expect("setup");
...
#[test]
fn client_does_not_install_store_gc_units() {
    let th = TempHome::new();
    th.write_minio_env("AK", "SK");
    let r = runner();
    setup_with(&r, &th.paths, &opts(Role::Client, &th)).expect("setup");
...
fn kache_daemon_already_installed_is_ignored() {
    ...
    let r = FakeRunner::new()
        ...
        .ok("systemctl", "");
    setup_with(&r, &th.paths, &opts(Role::Client, &th)).expect("already installed is fine");
}
...
fn shim_is_linked_to_self_exe() {
    ...
    setup_with(&r, &th.paths, &opts(Role::Client, &th)).expect("idempotent");
}
...
#[test]
fn server_role_never_recurses_to_remote() {
    let th = TempHome::new();
    th.write_minio_env("AK", "SK");
    let r = runner();
    let mut o = opts(Role::Server, &th);
